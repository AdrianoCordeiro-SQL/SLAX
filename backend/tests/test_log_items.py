...
from app.models import APILog, Account, User
...
def test_serialize_user_id_sem_linha_user_unknown(session: Session):
    """Se user_id não resolve (órfão), nome deve ser Unknown — se FK permitir inserir."""
    ...
    try:
        ...
        session.rollback()
        pytest.skip("SQLite/ORM impediu log com user_id órfão; ramo Unknown coberto no teste sem user_id.")
    session.refresh(log)
    row = serialize_api_log_row(session, log)
    assert row["user"] == "Unknown"
    assert row["avatar_url"] is None
